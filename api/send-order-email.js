...
export default async function handler(req, res) {
  ...
  const s = getEmailStrings(lang || 'lt');
  ...
  if (pdfBase64 && typeof pdfBase64 === 'string' && pdfBase64.length > 0) {
    try {
      ...
      if (buf.length > 0 && buf.length < 10 * 1024 * 1024) {
        attachments.push({
          filename: `${s.pdfFilename}-${vinStr || 'report'}-${new Date().toISOString().slice(0, 10)}.pdf`,
          content: buf,
        });
      }
    } catch (_) {}
  ...
  const reportsLink = token && n > 0
    ...
    <div style="margin:24px 0;padding:20px;background:#eef2ff;border:2px solid #4f46e5;border-radius:16px;">
      ...
      <p style="margin:0 0 16px 0;font-size:15px;color:#3730a3;">${generateNextText}</p>
      <p style="margin:0;"><a href="${baseUrl}/?token=${encodeURIComponent(token)}" style="display:inline-block;padding:14px 24px;background:#4f46e5;color:white;font-weight:bold;font-size:16px;text-decoration:none;border-radius:12px;">${s.viewRemaining}</a></p>
  ...
  const orderInfo = orderIdStr
    ? `<p style="margin:0 0 8px 0;font-size:14px;color:#64748b;">${s.orderLabel} <strong style="color:#1e293b;">${orderIdStr}</strong></p>`
    : '';
  ...
  const html = `
    ...
    <p>${s.viewOnSite} <a href="${baseUrl}">vinscanner.eu</a>.</p>
    ${reportsLink}
