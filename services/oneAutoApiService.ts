...
/** EzyVIN Service History – asinchroninis: 202 Accepted, tada pollinti. Laukiama max ~120s (40×3s), po to – „istorija nerasta“. */
async function fetchServiceHistory(
  ...
  const { maxPollAttempts = 40, pollIntervalMs = 3000 } = options ?? {};
...
/** EzyVIN VIN Lookup – OE duomenys iš VIN (gali būti 202). Laukiama max ~120s (40×3s). */
...
export async function fetchCarReportFromOneAuto(
  ...
  const pollOpts = { maxPollAttempts: options?.maxPollAttempts ?? 40, pollIntervalMs: options?.pollIntervalMs ?? 3000 };
