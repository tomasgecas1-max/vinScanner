...
const DiscountBanner: React.FC<DiscountBannerProps> = ({ t, onGetDiscountClick }) => {
  ...
  const bannerBase = 'bg-gradient-to-r from-rose-600 via-red-600 to-rose-500 shadow-md border-b border-rose-700/30';
  if (pendingDiscount) {
    return (
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-3 sm:py-3.5">
        <div className="flex flex-row flex-nowrap items-center justify-center">
          <button
            ...
            className="flex items-center gap-1.5 sm:gap-2 px-4 sm:px-5 py-2.5 sm:py-3 bg-white/25 hover:bg-white/35 rounded-xl text-white text-sm sm:text-base font-bold transition-colors shadow-sm border border-white/20"
          >
